...
If you are an active lab member, you are expected to
...
\item \textbf{Project meetings.}  Many of our collaborative
  ...
  \item \textbf{One-on-one meetings.} \director~holds regular
  one-on-one meetings with lab members who request them.  These
  are scheduled as part of the term scheduling process (below)
  and typically take place in Moore 349 or on Zoom.

  \item \textbf{Term scheduling process.}  Prior to the start of
  each term, \director~will post a scheduling survey on Slack.
  All active lab members are expected to:
  \begin{enumerate}
    \item Fill out their weekly availability for the upcoming term.
    \item Indicate which project meetings they plan to attend.
    \item Indicate whether they would like to schedule regular
      one-on-one meetings with \director, and if so, specify
      the preferred frequency (weekly or biweekly) and duration.
  \end{enumerate}
  Based on the collected responses, \director~will run a scheduling
  algorithm to find optimal meeting times for lab meetings, project
  meetings, and one-on-one meetings.  The finalized schedule is posted
  and pinned to the \texttt{\#general} Slack channel and added to the
  CDL Google Calendar.
...
% Roles with no current members (Lab Managers, Research Assistants) have no
% heading here on purpose. A `list` environment with no \item is a fatal LaTeX
% error -- "Something's wrong--perhaps a missing \item" -- so an empty
% placeholder block would stop the manual from building at all. Onboarding
% creates the heading and its list together, with the new member already in
% it; offboarding the last member removes the block again. See
% scripts/parse_lab_manual.py in the website repo.
...
\item Tom Hao Chang (2016 -- 2017)
\item Kirsten Ziman (2017 -- 2021)
\item Deepanshi Shokeen (2018 -- 2020)
...
\item Maddy Lee (2020 -- 2021)\end{list}
\end{multicols}
...
\item Jaysen Quan (2025 -- 2025)
\item Joy Maina (2025)
\item Keene Dampal (2025 -- 2025)
\item Luca Gandrud (2025 -- 2026)\end{list}
\end{multicols}
\end{fullwidth}
...
\chapter{Checklist and signature page}
  ...
  \item I agree to regularly attend lab meetings (unless I have a
    ...
    \item I agree to fill out the term scheduling survey prior to the
    start of each term, including my weekly availability and project
    meeting preferences.
